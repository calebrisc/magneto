...
- **Sensor orientation (worked out 8/19 while designing the M1 sled):** the field above a striped pattern rotates in the *vertical* plane containing the travel axis, and AAT-class parts are in-plane angle sensors — so the chip plane must stand **vertical, edge-on to the pad**, not lie flat (a flat chip sees only a 1-axis wobble; atan2 needs the rotating pair). This is why NVE's EVB01 boards put the chip at the board end. Consequence for the mouse: the three sensor sites at the floor are small vertical boards (or vertical-mount packages), and each site's chip plane must align with… a fixed axis per site — 2D/yaw solving from three fixed orientations is part of what M2 must prove. Deliberately bypasses integrated encoder ICs (AS5311-class) whose internal pipelines cap speed around ~650 mm/s; raw-bridge + firmware math has 10–20× margin over human flick speeds.
...
**In-amps are OPTIONAL for M1 (verified 8/19 — none were purchased and that's fine):** the AAT003's TMR bridges output ~200 mV/V (≈±330 mV at 3.3 V, mid-rail centered), which the nRF54L15 SAADC reads directly; the 40 kΩ source impedance just requires the long acquisition-time setting (still ~20 kHz/channel — 10× margin over hand-fling signal rates, and a scope's 1 MΩ input reads the raw bridge fine too). This is exactly why AAT003 (40 kΩ) was preferred over AAT001 (1.25 MΩ — would NOT work ampless; do not substitute). Amps re-enter for SNR headroom and M2's 6 channels — leave perfboard space, myTI samples are free.

**M1 sled CAD:** `cad/m1_sled/m1_sled.scad` (OpenSCAD, parametric) — puck + clamping wedge + notch-coded gap shims + slot-fit coupon. Board mounts vertical, sensor end down (see orientation note above); gap set by resting the board on a shim stack placed on the strip, clamping the wedge, then sliding the shims out.
...
## Feature concept: absolute-angle mode + lineup recall (post-M2, firmware-only)

Recorded 2026-08-19 from a design session. Requires nothing beyond the planned M2 hardware — this is firmware + companion app on top of working tracking. Not a v1/M-phase task; noted so it isn't lost.

- **Core idea:** the pad is a fixed encoder scale, so firmware can maintain a drift-free map of pad position ↔ view angle (X = yaw wrapping at pad edges, Y = pitch clamped ±89°). Pick DPI/sens so pad width = exactly 360° (counts per 360 in CS = 360/(0.022×sens)). Moment-to-moment aim is mathematically identical to a relative mouse (linear map ⇒ deltas preserved); the win is *known-angle* tasks, not duels.
- **Registration:** one manual anchor press per session (crosshair on a designated landmark) welds angle space to world space. Never read game state — everything stays in-firmware emitting ordinary HID deltas, which keeps it VAC-clean by construction. **Clutch-don't-lift discipline preserves registration on the periodic pattern** (sensors never leave the pad, so no lost-cell ambiguity); an absolute-coded pattern (vernier second pitch or PRBS track, writable with the jig) is the v2 upgrade that survives true lifts.
- **Killer app — utility lineup recall:** a lineup is (standing spot, exact angle); the mouse stores the angle half (8 bytes/lineup — storage is nothing). Record: stand on spot, aim it manually once, hold record. Recall: face roughly the right way, hold recall — firmware matches the *nearest stored angle* (rough aim is the menu) and guides via **haptic ticks** (metal-detector feel; guidance not auto-snap — auto-snap is an undetectable but macro-gray hardware automation, guidance keeps the human doing 100% of the movement).
- **Angle collisions** (skybox throws compress pitch, so distinct lineups can share a yaw neighborhood — e.g. Mirage T-spawn→CT vs deep-apps→market): companion app flags colliding pairs at sync time; each lineup gets a distinct haptic confirmation signature; EM-detent scroll wheel cycles candidates on an ambiguity buzz (stable yaw-sorted order). Failure mode is soft — eyes still verify against the sky before the throw.
